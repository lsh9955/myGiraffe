...
public class ImageUrlProvider {
  ...
  /**
   * 이미지 파일을 store-service 에 http 요청으로 보내고, 응답으로 온 BaseResponseBody 에서 이미지 url ("content")를 추출하여
   * 반환하는 메소드입니다.
   *
   * @param image 이미지 파일
   * @return 이미지 GET url
  ...
  public String getImageUrl(MultipartFile image) throws IOException {
    ...
    var type = objectMapper.getTypeFactory()
        .constructParametricType(BaseResponseBody.class, String.class);
  ...
  private String sendMultiPartFileHttpRequest(MultipartFile imageFile) throws IOException {
    ...
    try (var response = okHttpClient.newCall(request).execute()) {
      return Optional.ofNullable(
              response.body()) // response.body 는 한번 호출하면 다음부터는 null 을 반환함 (consume only once).
          .map((body) -> {
            try {
              return body.string();
            } catch (IOException e) {
